...
void prepare_buffers(ICudaEngine* engine, float** gpu_input_buffer,
  ...
  CUDA_CHECK(cudaMalloc((void**)gpu_input_buffer,
                        1 * 3 * kInputH * kInputW * sizeof(float)));
  CUDA_CHECK(
      cudaMalloc((void**)gpu_output_buffer, 1 * kOutputSize * sizeof(float)));

  *cpu_output_buffer = new float[1 * kOutputSize];
}
...
std::pair<cv::Mat, std::vector<Detection>> run_inference(IExecutionContext *context, cv::Mat& img, float** gpu_buffers, float* cpu_output_buffer, cudaStream_t &stream) {
  std::vector<cv::Mat> img_batch;
  std::vector<std::string> img_name_batch;
  img_batch.push_back(std::move(img));
  ...
  return {img_batch[0], res_batch[0]};
}

int main(int argc, char** argv) {
  cudaSetDevice(kGpuId);

  std::string wts_name = "/tmp/best.wts";
  std::string engine_name = "/tmp/best.engine";
  bool is_p6 = false;
  ...
  ros::init(argc, argv, "cuda_yolo");
  ros::NodeHandle nh;
  auto image_pub = nh.advertise<sensor_msgs::Image>("yolo/annotated_image", 1);
  auto image_sub = nh.subscribe<sensor_msgs::Image>("/video_source/raw", 1, [&](const sensor_msgs::ImageConstPtr& msg){
    cv_bridge::CvImagePtr cv_ptr;
    cv_ptr = cv_bridge::toCvCopy(msg, sensor_msgs::image_encodings::BGR8);
    //cv::Mat img;
    cv::Mat img = cv_ptr->image;
    //cv::cvtColor(img, img, CV_RGB2BGR);
    //cv::resize(orig_img, img, cv::Size(640,640));
    auto out = run_inference(context, img, gpu_buffers, cpu_output_buffer, stream);
    auto annotated = out.first;
    auto detections = out.second;

    auto out_msg = cv_bridge::CvImage(std_msgs::Header(), "bgr8", annotated).toImageMsg();
    image_pub.publish(out_msg);
    for(int i = 0; i < detections.size(); i++){
    	int id = detections[i].class_id;
	cv::Rect box = get_rect(annotated, detections[i].bbox);
	std::cout<< "DETECTION: " << id << " x: " << box.x << " y: " << box.y << std::endl;
    }
    ROS_INFO("DONE");
  });
  ros::spin();
